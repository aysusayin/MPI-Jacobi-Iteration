...
```
The domain is [0,1] and the value of the boundary points are given. The program takes argument N as input and divide the domain into a (n+1) x (n+1) + (n+1) cube grid. The approximation of each grid point is
```
...
The value of the boundary points are calculated with exact() function. In this homework, exact function is given as f = x*y*z.
